    ...
    public void render(ForgeIngameGui gui, PoseStack poseStack, float partialTick, int width, int height) {
        ...
        if (!UmapyoiAPI.getUmaSoul(player).isEmpty()) {
            ...
            case BAD -> {
                ...
                this.minecraft.font.draw(poseStack, new TranslatableComponent("umapyoi.motivation.bad"), x + 132,
                        y - 34, 0XFFFFFF);
            }
            case DOWN -> {
                GuiComponent.blit(poseStack, x + 118, y - 37, 0, 45, 64, 14, 64, 96);
                this.minecraft.font.draw(poseStack, new TranslatableComponent("umapyoi.motivation.down"), x + 132,
                        y - 34, 0XFFFFFF);
            }
            case NORMAL -> {
                GuiComponent.blit(poseStack, x + 118, y - 37, 0, 30, 64, 14, 64, 96);
                this.minecraft.font.draw(poseStack, new TranslatableComponent("umapyoi.motivation.normal"), x + 132,
                        y - 34, 0XFFFFFF);
            }
            case GOOD -> {
                GuiComponent.blit(poseStack, x + 118, y - 37, 0, 15, 64, 14, 64, 96);
                this.minecraft.font.draw(poseStack, new TranslatableComponent("umapyoi.motivation.good"), x + 132,
                        y - 34, 0XFFFFFF);
            }

            case PERFECT -> {
                GuiComponent.blit(poseStack, x + 118, y - 37, 0, 0, 64, 14, 64, 96);
                this.minecraft.font.draw(poseStack, new TranslatableComponent("umapyoi.motivation.perfect"), x + 132,
                        y - 34, 0XFFFFFF);
            }
