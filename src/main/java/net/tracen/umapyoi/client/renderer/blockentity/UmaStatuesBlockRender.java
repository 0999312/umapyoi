...
public class UmaStatuesBlockRender
		implements BlockEntityRenderer<UmaStatueBlockEntity>, BlockEntitySectionGeometryRenderer<UmaStatueBlockEntity> {
...
//		}
	...
	private void renderModel(UmaStatueBlockEntity tileEntity, Direction direction, SectionRenderingContext context,
			PoseStack poseStack, BlockPos pos, BlockPos regionOrigin, SectionGeometryRenderContext renderContext) {
		...
		ResourceLocation target = tileEntity.isEmpty() ? TEXTURE : ClientUtils.getTexture(UmaSoulUtils.getName(item));
	...
	@Override
	public void renderSectionGeometry(UmaStatueBlockEntity tileEntity, SectionRenderingContext context,
			PoseStack poseStack, BlockPos arg3, BlockPos arg4, SectionGeometryRenderContext renderContext) {
		Level world = tileEntity.getLevel();
		boolean flag = world != null;
		BlockState blockstate = flag ? tileEntity.getBlockState() : BlockRegistry.UMA_STATUES.get().defaultBlockState();
		if (blockstate.getBlock() instanceof UmaStatueBlock) {
