...
public class FactorDecomposeMenu extends AbstractContainerMenu {
    ...
    public static boolean AllowContinueDefaultLogic(ItemStack factorStackCandidate) {
        ...
        List<FactorData> factors = factorStackCandidate.get(DataComponentsTypeRegistry.FACTOR_DATA);
        if (factors == null) return false;
        return UmaFactorUtils.deserializeData(factors).stream().anyMatch(opt -> opt.getLevel() > 0);
    }

    public void createResult() {
        if (isTaking()) return;
        FactorDecomposeEvent evt = new FactorDecomposeEvent(this.inputSlots.getItem(0), this.factorSeed.get());
        if (!this.canProceedToResult() || NeoForge.EVENT_BUS.post(evt).isCanceled() || evt.getListOfReturn() == null) {
            ...
            List<ItemStack> returnStacks = evt.getListOfReturn().stream().map(s -> {
                ...
                returnStack.set(DataComponentsTypeRegistry.FACTOR_DATA, UmaFactorUtils.serializeData(List.of(s)));
                return returnStack;
            }).toList();
    ...
    public static List<UmaFactorStack> DefaultAlgResultStacks(ItemStack factorStack, int seed) {
        ...
        random.setSeed(seed);
        List<UmaFactorStack> listOfFactor = UmaFactorUtils.deserializeData(factorStack.getOrDefault(DataComponentsTypeRegistry.FACTOR_DATA, List.of())).stream().filter(s -> s.getFactor().getFactorType() != FactorType.UNIQUE).collect(Collectors.toCollection(ArrayList::new));
