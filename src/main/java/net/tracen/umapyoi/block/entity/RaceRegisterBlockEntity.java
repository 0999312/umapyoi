...
public class RaceRegisterBlockEntity extends SyncedBlockEntity implements MenuProvider {
    ...
    public RaceRegisterBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.IOHandler = new UniversalIOItemHandler(inventory, 2);
    ...
    @Override
    public void loadAdditional(@Nonnull CompoundTag compound, HolderLookup.Provider registries) {
    ...
    @Override
    public void saveAdditional(@Nonnull CompoundTag compound, HolderLookup.Provider registries) {
    ...
    public CompoundTag getUpdateTag(HolderLookup.Provider registries) {
        CompoundTag returnVal = new CompoundTag();
        saveAdditional(returnVal, registries);
        return returnVal;
    }
    ...
    public static void registerCapabilities(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(
                ...
                BlockEntityRegistry.RACE_REGISTER_BLOCK_ENTITY.get(),
                (be, ctx) -> {
                    return be.IOHandler;
                }
        );
    ...
    public ItemStack insertItemToSlot(int slot, @NotNull ItemStack stack)
    {
        ...
        int limit = existing.isEmpty() ? this.inventory.getSlotLimit(slot) : Math.min(this.inventory.getSlotLimit(slot), existing.getMaxStackSize());
        ...
        if (existing.isEmpty()) {
            this.inventory.setStackInSlot(slot, reachedLimit ? stack.copyWithCount(limit) : stack);
        } else {
    ...
    private LootTable getLootTable(ResourceLocation lootSpecify) {
        return Objects.requireNonNull(this.level.getServer()).reloadableRegistries().getLootTable(ResourceKey.create(Registries.LOOT_TABLE, lootSpecify));
    }
    ...
    public ItemStack getResultItem(ResourceLocation raceID) {
        ...
        LootTable table = getLootTable(lootSpecify);
        if (table == LootTable.EMPTY) {
            ...
            if (stackSoul.equals(ItemStack.EMPTY)) {
                ...
                table = getLootTable(ResourceLocation.fromNamespaceAndPath(Umapyoi.MODID, "race/generic/race_" +
                        rank.name().toLowerCase()));
            } else {
                ResourceLocation field = race.field(this.level, stackSoul).id();
                table = getLootTable(ResourceLocation.fromNamespaceAndPath(field.getNamespace(), "race/generic/field/race_"
