...
public class UmaStatusUtils {
    ...
    public static void changeMotivation(@Nullable LivingEntity entity, int addMotivLevel) {
        ...
        if (NeoForge.EVENT_BUS.post(evt).isCanceled()) return;

        if (evt.getDoTriggerBonus() && entity != null) {
            int level = Optional.ofNullable(entity.getEffect(MobEffectRegistry.MOOD_BONUS))
                    .map(MobEffectInstance::getAmplifier).orElse(-1) + 1;
            entity.addEffect(new MobEffectInstance(MobEffectRegistry.MOOD_BONUS, 1200, level));
        }
